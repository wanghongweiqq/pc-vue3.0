...
<template>
  <div class="content">
    ...
    <p>var变量不受{}块级作用域限制，在循环/条件块中声明的变量会泄露到外部</p>
    <p>允许在同一作用域内多次var声明同一变量，后续声明的变量只要有值就会覆盖前面的同名变量，无值时不会覆盖前面已赋的值。</p>

    <h3>2、函数声明作用域提升</h3>
    <p>非严格模式下（ES5及其以前），浏览器的控制台默认就是此模式，函数内使用（没有块级作用域的包裹），声明和赋值都会提升到函数作用域内的顶部，原位置相当于被删除。</p>
    <p>非严格模式下，函数内的块级作用域使用，声明会提升到函数作用域内的顶部，赋值会提升到块级作用域内的顶部，原位置相当于被删除。</p>
    <p>严格模式下（ES6+），vue和react内都是严格模式。声明和赋值都会提升到最近作用域内的顶部。</p>
  </div>
</template>
<script setup>
...
function demoA1 () {
  ...
  console.log(-1, a) // -1 undefined 不知此时的undefined是变量提升导致的还是函数提升导致的，反正他俩都会导致提升，在只有任何一个单独存在时都成立（函数提升在非严格模式下，在严格模式下直接就报引用错误了）
  var a = 0
  console.log(0, a) // 0 0
  if(true) {
